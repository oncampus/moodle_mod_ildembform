...
class ildembform_sendmail
{
    ...
    public function sendmessage($subject, $message, $cid, $receivers, $url, $anonymized) {
        ...
        $htmlmessage = '';
        $htmlmessage .= '<p><strong>' . get_string('mailsubject', 'ildembform') . '</strong><br>' . $subject . '</p>';
        $htmlmessage .= '<p><strong>' . get_string('mailfromcourse', 'ildembform') . '</strong><br>' .
            $fromcourse . '<br />' . $url . '</p>';
        $htmlmessage .= '<p><strong>' . get_string('mailmessage', 'ildembform') . '</strong><br>' . $message . '</p>';
        $htmlmessage .= '<hr />';
        $htmlmessage .= '<p><strong>' . get_string('mailfrom', 'ildembform') . '</strong><br>' . $fromfirstname . ' ' .
            $fromlastname . '<br>' . $frommail . '</p>';

        // If all required data exists, check $receivers if is array or string.
        if (is_array($receivers)) {
            foreach ($receivers as $rec) {
                $userobjectto = self::create_touser($rec, $CFG->supportemail, '', '', '-99', 1, true);
                $userobjectfrom = self::create_fromuser($frommail, $fromfirstname, $fromlastname, $fromusername,
                    $USER->id, 1, true);

                if (email_to_user($userobjectto, $userobjectfrom, $subject, html_to_text($htmlmessage), $htmlmessage,
                    true, '', '')) {
                    $send = true;
                } else {
                    $send = false;
            ...
            $userobjectto = self::create_touser($receivers, $CFG->supportemail, '', '', '-99', 1, true);
            $userobjectfrom = self::create_fromuser($frommail, $fromfirstname, $fromlastname, $fromusername,
                $USER->id, 1, true);
    ...
    private static function create_fromuser($receiver, $firstname, $lastname, $username = '', $id = '',
                                            $mailformat = 1, $maildisplay = true) {
    ...
    private static function create_touser($receiver, $firstname, $lastname, $username = '', $id = '',
                                          $mailformat = 1, $maildisplay = true) {
